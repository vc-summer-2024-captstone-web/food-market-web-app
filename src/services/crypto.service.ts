...
const { CONTACT_ENCRYPTION_KEY } = import.meta.env;
...
export const encryptBody = (body: string): string => {
  ...
  const cipher = createCipheriv('aes-256-cbc', Buffer.from(CONTACT_ENCRYPTION_KEY), iv);
...
export const decipherBody = (encryptedBody: string): string => {
  try {
    ...
    const decipher = createDecipheriv('aes-256-cbc', Buffer.from(CONTACT_ENCRYPTION_KEY), iv);
